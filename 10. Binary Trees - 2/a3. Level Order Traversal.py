...
Sample Input 2:     8 3 10 1 6 -1 14 -1 -1 4 7 13 -1 -1 -1 -1 -1 -1 -1
Sample Output 2:    8
                    3 10
                    1 6 14
                    4 7 13
...
def printLevelWise(root):
    ...
    while not q.empty():
        current = q.get()           # front element
        if current != None:
            print(current.data, end=" ")

            if current.left != None:                # store left/right children of current node in queue, if any
                q.put(current.left)
            ...
            if q.empty():           # to stop infinite loop
                break
            q.put(None)
