...
Sample Input 1:     8 5 10 2 6 -1 -1 -1 -1 -1 7 -1 -1
                    6 10
Sample Output 1:    6 7 8 10
'''
...
def elementsInRangeK1K2(root, k1, k2):
    ...
    if root.data > k1:  #If the value of root’s key is greater than k1, then recursively call in left subtree.
        elementsInRangeK1K2(root.left, k1, k2)


    if root.data >= k1 and root.data <= k2:     # If root's data lies in range, then prints root's data
        print(root.data, end=" ")

    if root.data < k2:      # If root.data is smaller than k2, then only we can get o/p keys in right subtree
