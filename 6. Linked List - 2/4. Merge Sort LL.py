...
def midpoint(head):     # func. to find mid point of LL
    if head is None:
        return head
    slow = head
    fast = head
...
def mergeTwoLL(head1, head2):   # func. to merge two Sorted LL
...
def mergeSort(head):
    ...
    head1 = head                # start of 1st half
    mid = midpoint(head)        # mid point of LL, break LL in 1st & 2nd half
    head2 = mid.next            # start pt. of 2nd half
    mid.next = None             # last pt. of 1st half

    leftHalf = mergeSort(head1)     # apply
    rightHalf = mergeSort(head2)

    sortedLinkedList = mergeTwoLL(leftHalf, rightHalf) # merge 1st & 2nd half
